...
load('Resultados mtarns.RData')
load('data_c.RData')
load('ns2.RData')
library(tidyverse)

rls2 = function (y, x, ist = 30, xpxi = NULL, xpy0 = NULL){
  x = as.matrix(x)
  y = as.matrix(y)
  nT = dim(x)[1]
  k = dim(x)[2]
  ...
  xpxi = solve(xpx0)
  xpy0 = t(x[1:ist, ]) %*% matrix(y[1:ist,], ist, m)
  ist = ist + 1
  beta = resi = list()
  beta[[1]] = matrix(c(xpxi %*% xpy0), k, m)
  for (t in ist:nT) {
    jdx = t - ist + 1
    x1 = matrix(x[t, ], 1, k)
    wk = x1 %*% xpxi
    ...
    xpxi = xpxi - wk1
    Kmtx = xpxi %*% t(x1)
    tmp = beta[[jdx]] - Kmtx %*% (x1 %*% beta[[jdx]] - y[t,])
    beta[[jdx + 1]] = tmp
    res = y[t,] - x1 %*% tmp 
    resi[[jdx]] = res
  }
...
Ctest = function(X,Y = NULL,Z = NULL,p = 1,d = 0, q = 0,m = 30){
  h = max(c(p,q,d))
  X = as.matrix(X)
  Tlen = dim(X)[1]
  k = dim(X)[2]
  if (is.null(Y)){nu = 0}else{nu = dim(Y)[2]}
  X = matrix(X,ncol = k,nrow = Tlen)
  if (!is.null(Z)){
    reg = data.frame(X = X[-c(1:h),] %>% as.matrix,
                 W = X[-c((Tlen - h + 1):Tlen),] %>% as.matrix,
                 Z = Z[-c(1:h)])
    reg2 = reg %>% arrange(Z)
  }else{
    reg2 = data.frame(X = X[-c(1:h),]%>% as.matrix,
                 W = X[-c((Tlen-h+1):Tlen),]%>% as.matrix)
  }
  xt = (reg2[,1:k] %>% as.matrix())
  wt = cbind(rep(1,Tlen - h),reg2[,((k + 1):(2*k))] %>% as.matrix())
  beta = rls = list()
  resi = rls2(xt,wt,ist = m)$resi
  Vm = 0
  et = etat = ept = matrix(ncol = Tlen - h - m,nrow = k)
  for (t in 1:(Tlen - h - m)) {
    et[,t] = resi[[t]]
    Vm = Vm + (c(wt[t,])%*%t(wt[t,]))
  }
  Vm1 = solve(Vm)
  for(t in 1:(Tlen - h - m)){
    etat[,t] = et[,t]*as.numeric(1/(1 + t(wt[t,])%*%Vm1%*%wt[t,])^{1/2})
  }
  #
  m0 = m
  for (o in 1:k){
    data1 = data.frame(wt[-c(1:m0),], y = etat[o,])
    ept[o,] = lm(y~.-1,data = data1)$residuals
  }
  Seta = Sep = 0
  for (t in 1:(Tlen - h - m0)){
    Seta = Seta + etat[,t]%*%t(etat[,t])
    Sep = Sep + ept[,t]%*%t(ept[,t])
  }
  S0 = 1/(Tlen - h - m0)*Seta
  S1 = 1/(Tlen - h - m0)*Sep
  C = (Tlen - h - m0 - (k*p + q*nu + 1))*(log(det(S0)) - log(det(S1)))
  pval = pchisq(C,k*(k*p + q*nu + 1),lower.tail = F)
  a = c('test' = C,'p-value' = pval)
  return(a)
}
```
...
autoplot(estim_nr$list_m$m2$par,4)$R1$phi + autoplot(estim_nr$list_m$m2$par,4)$R2$phi
```

Observando en los resultados del procedimiento el comportamiento de las variables gammas (variables indicadoras relacionadas con los ordenes) y sus frecuencias más comunes (en el primer régimen la mayor frecuencia fue de 22.25\% seguida del 5.5\% y en el segundo régimen la mayor de 6.3\% seguida de 6.15\%), se tienen las estimaciones de ordenes para el modelo con $p_1 = 5,d_1 = 5$ para el primer régimen y $p_2 = 5,d_2 = 5$ para el segundo además de una estimación para $r = 2.6$. Luego de encontrar un aproximación a los ordenes de los rezagos (parámetros estructurales) así como del valor del umbral es posible una primera estimación de los datos faltantes (por medio de la metodología del filtro de Kalman y el modelo espacio-estado), esto a partir de la función ``mtarmissing()'' que realiza la estimación de datos faltantes realizando el proceso de muestreo conjunto de parámetros no estructurales y observaciones faltantes. En la gráfica observamos el valor puntual y su respectivo intervalo de credibilidad al 95\% en azul para los valores faltantes.

```{r,fig.height = 3,fig.width=10,warning=FALSE}
# list_model = list(pars = list(l = 2,r = 2.6,
# orders = list(pj = c(5,5),dj = c(5,5))))
# initial = mtarinipars(tsregime_obj = tsregime(Yt = data_miss$Yt,
# Zt = data_miss$Zt),list_model = list_model)
# missingest = mtarmissing(ini_obj = initial,chain = TRUE, niter = 5000,
# burn = 1000)
...
# initial = mtarinipars(tsregime_obj = data_c,
# list_model = list(pars = list(l = 2,
# orders = list(pj = c(5,5),dj = c(5,5)),r = 2.6)))
# ns2 = mtarns(ini_obj = initial,niter = 15000,chain = TRUE,burn = 5000)
...
Finalmente vemos la estimación de los parámetros con valor de umbral 2.6 y en el gráfico el ajuste del modelo a la serie de datos una vez completados los valores faltantes.
