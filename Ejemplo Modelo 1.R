...
initial = mtarinipars(tsregime_obj = data_temp,method = 'KUO',
                      list_model = list(pars = list(l = estim_nr$final_m),
                                        orders = list(pj = c(2,2),qj = c(2,2),dj = c(2,2))))
estruc = mtarstr(ini_obj = initial,niter = 500,chain = TRUE,parallel = FALSE,burn = 100)
...
initial = mtarinipars(tsregime_obj = data_miss,list_model = list_model)
