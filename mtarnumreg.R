rm(list = ls()[ls() != c("Yt","Ut")])
#---Lectura de las simulaciones--#
l0 = 3
niter = 6000
chain = T
level = 0.95
...
#a = quantile(Zt,probs = 0.1)
a = min(Zt)
b = max(Zt)
#b = quantile(Zt,probs = 0.9)
...
### Crear las iniciales de cada modelo
...
#--Proceso iterativo--#
  ...
  rpseudo = function(l,...){
    ...
    for (lj in 1:l) {
      ...
      sigma_iter[[lj]][[1]] = 1/nu0jS[[lj]]*MCMCpack::rwish(v = nu0jS[[lj]] ,S = S0jS[[lj]])
  ...
  prodA = function(thetaym, thetaymp, basem, basemp){
    ...
    # Valor del numerador
    for (lj in 1:lmp) {
      #pijSmp[[lj]][pijSmp[[lj]] == 1] = 0.999
      #pijSmp[[lj]][pijSmp[[lj]] == 0] = 0.00005
    ...
    for (lj in 1:lm) {
      #pijSm[[lj]][pijSm[[lj]] == 1] = 0.999
      #pijSm[[lj]][pijSm[[lj]] == 0] = 0.00005
  ...
  for (im in 2:{niter + burn}) {
    ...
    # Generamos valor para Theta_m
    listTemp = updatelist(l = m_iter[im - 1])
    # Generamos valor para Thema_mp
