...
mtarns = function(ini_obj, level = 0.95, burn = NULL, niter = 1000, chain = FALSE, r_init = NULL){
  ...
  #   rj = matrix(nrow = 2,ncol = l)
  #   rj[,1] = c(-Inf,r_fit[1])
  #   rj[,l] = c(rev(r_fit)[1],Inf)
  #   if (l > 2) {
  #     for (i in 2:{l - 1}) {rj[,i] = c(r_fit[i - 1],r_fit[i])}
  #   }
  # }
  # # initial Vectors
  # maxj = max(pj,qj,dj)
  # Yt_fit = matrix(0,nrow = k,ncol = N + maxj)
  # Yt[,1:maxj] = NA
  # Zt = c(rep(0,maxj),Ut[1,])
  # if (nu == 0) {
  #   Xt = matrix(0,ncol = N + maxj,nrow = 1)
  # }else{
  #   Xt = cbind(rep(0,nu) %x% matrix(1,ncol = maxj),matrix(Ut[-1,],nrow = nu))
  # }
  # for (i in (maxj + 1):(N + maxj)) {
  #   ## evaluate regime
  #   for (w in 1:l) {
  #     if (Zt[i] > rj[1,w] & Zt[i] <= rj[2,w]) {Ri = Rest[[w]]}
  #   }
  ...
  #   yti = c()
  #   for (w in 1:p) {yti = c(yti,Yt_fit[,i - w])}
  #   xti = c()
  #   if (l == 1 & nrow(Ut) != 1) {
  #     xti = c(xti,Xt[,i])
  #   }else{
  ...
  #   Yt_fit[,i] = cs + At %*% yti + Bt %*% xti + Dt %*% zti
  #   Yt_fit[,i] = solve(Sig) %*% Yt_fit[,i]
  # }
  # # delete burn
  # if (k == 1) {
  #   Yt = as.matrix(Yt[-(1:{maxj})])
  # }else{
  #   Yt = t(Yt[,-(1:{maxj})])
  # }
  # Zt = Zt[-c(1:{maxj})]
  # if (nu == 1) {
  #   Xt = as.matrix(Xt[,-c(1:{maxj})])
  # }else{
  #   Xt = t(Xt[,-c(1:{maxj})])
  # }
  # # residuals
