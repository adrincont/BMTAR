...
autoplot.tsregim(Yzz$Sim,1)
posNA = sample(c(1:1000),4)
datasim$Sim$Yt[c(posNA),] = c(NA,NA)
posNA = sample(c(1:1000),4)
Ut = tsregim(cbind(datasim$Sim$Zt,datasim$Sim$Xt))
Ut$Yt[c(posNA),] = c(NA,NA)
autoplot.tsregim(Ut)
...
modelU = mtarns(tsregim(t(Ut)),orders = list(pj = b,qj = 0,dj = 0),niter = 2000,chain = FALSE,burn = 1000)
...
state_space = function(reg, iSS, theta, sigma, ...) {
  ...
  Aj = cbind(Aj[,1],Aj[,-1][,1:(p*k)], matrix(0,nrow = k, ncol = k*(pmax - p)),
             Aj[,-1][,(p*k + 1):(p*k + q*nu)],matrix(0,nrow = k, ncol = nu*(qmax - q)),
             Aj[,-1][,(p*k + q*nu + 1):(p*k + q*nu + d)],matrix(0,nrow = k,ncol = dmax - d))
  paux = ifelse(p == 1,2,p)
  qaux = ifelse(q == 0,1,q)
  daux = ifelse(d == 0,1,d)
...
for (i in 2:{niter + burn}) {
  ...
  for (i1 in PosNAvec[[1]]) {
    Ysim = as.matrix(MASS::mvrnorm(1,AlphaT[[i1 + 1]][1:k],PT[[i1 + 1]][1:k,1:k]))
...
>>>>>>> 629a3da9536e9bc08b1300720d39554fa857fd8b
close(pb)