autoplot.tsregim = function(object, type = 1 , ...) {
  ...
  if (!is.null(object$Zt)) {
    ...
    p2 = p2 + ggplot2::labs(title = "Threshold processes",
...
autoplot.regime_model = function(object, type = 1 , ...) {
  ...
  Chains_r = t(object$Chain$r)
  time = seq(1,nrow(Chain_r))
  dat2 = data.frame(name = 'r1',time = time,value = Chains_r[,1])
  if (ncol(Chains_r) > 1) {
    for (i in 2:ncol(Chains_r)) {
      dat2 = rbind.data.frame(dat2,data.frame(name = paste0('r',i),
                                   time = time,value = Chains_r[,i]))
    }
  }
  ggplot(data = dat2,aes(x = time,y = value))
  p3 = ggplot2::ggplot(data = dat2,ggplot2::aes(x = time,y = value))
  p3 = p3 + ggplot2::geom_line() + ggplot2::facet_grid(name~.) + theme_bw()
  p3 = p3 +  labs(title = "Chains threshold values")
